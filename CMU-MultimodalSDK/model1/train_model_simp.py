...
def main():
    ...
    print(txt_dim, aud_dim, vid_dim, ey_tr.shape[-1], "dims")
    ...
    classifier1 = LSTM(txt_dim, 2, 1)
    classifier2 = LSTM(txt_dim, 2, 1)
    multi_classi = LSTM(txt_dim+aud_dim+vid_dim, 2, 1)
    ...
    optimizers = []
    model_lr = 0.0001
    params = list(aud_en.parameters()) + list(vid_en.parameters()) + list(txt_en.parameters()) \
            +list(model.parameters()) + list(classifier1.parameters()) + list(classifier2.parameters()) +\
             list(multi_classi.parameters())
    ...
    op = optim.Adam(params, lr = model_lr)
    optimizers = [op]
